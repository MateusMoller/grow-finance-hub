...
import { User, Shield, Bell, BellOff, Palette, Building2, Globe, ChevronRight, Loader2, Upload, KeyRound, RefreshCcw, Smartphone } from "lucide-react";
...
const settingSections: { id: SettingSectionId; title: string; description: string; icon: typeof User }[] = [
  ...
  { id: "company", title: "Empresa", description: "Dados da organizaÃ§Ã£o", icon: Building2 },
  { id: "security", title: "SeguranÃ§a", description: "Senha e autenticacao", icon: Shield },
  { id: "notifications", title: "NotificaÃ§Ãµes", description: "Alertas do sistema", icon: Bell },
  { id: "appearance", title: "AparÃªncia", description: "Tema e idioma", icon: Palette },
  { id: "integrations", title: "IntegraÃ§Ãµes", description: "ConexÃµes externas", icon: Globe },
];

const notificationLabels = [
  { key: "assignedTasks", title: "Novas tarefas atribuÃ­das", desc: "Quando uma tarefa for atribuida a vocÃª" },
  { key: "dueSoon", title: "Prazo prÃ³ximo", desc: "Alerta de vencimento" },
  { key: "newForms", title: "Novos formulÃ¡rios", desc: "Quando cliente enviar formulario" },
...
export default function ConfiguracoesPage() {
    ...
    try {
      ...
      if (status.subscribed && user?.id) {
        try {
          ...
          // Mantem o status local caso a sincronizacao remota falhe.
        }
      }
    } catch {
  ...
  const saveNotifications = async () => {
    ...
    setSavingSection(null);
    if (error) return toast.error("Erro ao salvar notificaÃ§Ãµes.");
    toast.success("NotificaÃ§Ãµes salvas.");
  };
  ...
  const handleThemePreferenceChange = (value: ThemePreference) => {
    ...
    void upsertUserSettings({ theme_preference: normalized })
      .then(({ error }) => {
        if (error) {
          toast.error("NÃ£o foi possÃ­vel salvar o tema automaticamente.");
        }
      })
      .finally(() => {
  ...
  const saveAppearance = async () => {
    ...
    setSavingSection(null);
    if (error) return toast.error("Erro ao salvar aparÃªncia.");
    toast.success("AparÃªncia salva.");
  };
  ...
  const saveIntegrations = async () => {
    ...
    setSavingSection(null);
    if (error) return toast.error("Erro ao salvar integraÃ§Ãµes.");
    toast.success("IntegraÃ§Ãµes salvas.");
  };
  ...
  const changePassword = async () => {
    ...
    if (securityForm.newPassword !== securityForm.confirmPassword) return toast.error("ConfirmaÃ§Ã£o da senha invÃ¡lida.");
    ...
    if (signInError) {
      setSavingSection(null);
      return toast.error("Senha atual invÃ¡lida.");
    }
    const { error: updateError } = await supabase.auth.updateUser({ password: securityForm.newPassword });
    setSavingSection(null);
    if (updateError) return toast.error("NÃ£o foi possÃ­vel alterar a senha.");
    setSecurityForm({ currentPassword: "", newPassword: "", confirmPassword: "" });
    toast.success("Senha alterada com sucesso.");
  };
  const pushStatusLabel = (() => {
    if (!pushStatus.supported) return "Push não suportado neste navegador.";
    if (!pushStatus.hasPublicKey) return "Chave pública VAPID não configurada no app.";
    ...
    if (!pushStatus.subscribed) return "Push habilitado no navegador, mas sem inscrição ativa.";
  ...
  return (
    <AppLayout>
      <div className="space-y-6 max-w-5xl">
        <div><h1 className="font-heading text-2xl font-bold">ConfiguraÃ§Ãµes</h1><p className="text-sm text-muted-foreground">Gerencie seu perfil e preferÃªncias do sistema</p></div>
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeSection === "notifications" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">NotificaÃ§Ãµes</h2><div className="space-y-4">{notificationLabels.map((item) => <div key={item.key} className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">{item.title}</div><div className="text-xs text-muted-foreground">{item.desc}</div></div><Switch checked={notificationSettings[item.key]} onCheckedChange={(checked) => setNotificationSettings((prev) => ({ ...prev, [item.key]: checked }))} /></div>)}</div><Button className="mt-4" onClick={saveNotifications} disabled={savingSection === "notifications"}>{savingSection === "notifications" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar notificacoes</Button></div>}
            {activeSection === "notifications" && <div className="rounded-xl border bg-card p-6 space-y-3"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold flex items-center gap-2"><Smartphone className="h-4 w-4" />Notificações Push (PWA)</h3><p className="text-xs text-muted-foreground mt-1">{pushStatusLabel}</p></div><Button size="sm" variant="ghost" onClick={() => void loadPushStatus()} disabled={pushActionLoading !== null}><RefreshCcw className="h-3.5 w-3.5 mr-1" />Atualizar status</Button></div><div className="flex flex-wrap gap-2"><Button size="sm" onClick={() => void handleEnablePush()} disabled={pushActionLoading !== null || !pushStatus.supported || !pushStatus.hasPublicKey}>{pushActionLoading === "enable" && <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" />}Ativar no dispositivo</Button><Button size="sm" variant="outline" onClick={() => void handleDisablePush()} disabled={pushActionLoading !== null || !pushStatus.subscribed}>{pushActionLoading === "disable" ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : <BellOff className="h-3.5 w-3.5 mr-1" />}Desativar</Button><Button size="sm" variant="outline" onClick={() => void handleSendPushTest()} disabled={pushActionLoading !== null || !pushStatus.subscribed}>{pushActionLoading === "test" ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : <Bell className="h-3.5 w-3.5 mr-1" />}Enviar teste</Button><Button size="sm" variant={pushStatus.subscribed ? "default" : "outline"} disabled>{pushStatus.subscribed ? "Ativo" : "Inativo"}</Button></div>{pushStatus.permission === "denied" && (<p className="text-xs text-amber-700 dark:text-amber-300">O navegador bloqueou notificações. Libere nas configurações do site/app no celular.</p>)}{!pushStatus.hasPublicKey && (<p className="text-xs text-muted-foreground">Configure a variável <code>VITE_WEB_PUSH_PUBLIC_KEY</code> para habilitar inscrições push.</p>)}</div>}
            {activeSection === "appearance" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">AparÃªncia</h2><div className="grid sm:grid-cols-2 gap-4"><div className="space-y-2"><Label className="text-sm">Tema</Label><Select value={appearanceSettings.themePreference} onValueChange={handleThemePreferenceChange}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="system">Sistema</SelectItem><SelectItem value="light">Claro</SelectItem><SelectItem value="dark">Escuro</SelectItem></SelectContent></Select>{savingThemePreference && <p className="text-xs text-muted-foreground">Salvando tema...</p>}</div><div className="space-y-2"><Label className="text-sm">Idioma</Label><Select value={appearanceSettings.languageCode} onValueChange={(value) => setAppearanceSettings((prev) => ({ ...prev, languageCode: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="pt-BR">PortuguÃªs (Brasil)</SelectItem><SelectItem value="en-US">English (US)</SelectItem><SelectItem value="es-ES">Espanhol</SelectItem></SelectContent></Select></div></div><div className="mt-4 flex items-center justify-between gap-3 border rounded-lg p-3"><div><div className="text-sm font-medium">Modo compacto</div><div className="text-xs text-muted-foreground">Reduz espaÃ§os da interface</div></div><Switch checked={appearanceSettings.compactMode} onCheckedChange={(checked) => setAppearanceSettings((prev) => ({ ...prev, compactMode: checked }))} /></div><Button className="mt-4" onClick={saveAppearance} disabled={savingSection === "appearance" || savingThemePreference}>{savingSection === "appearance" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar aparÃªncia</Button></div>}
            {activeSection === "integrations" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">IntegraÃ§Ãµes</h2><div className="space-y-4"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">SincronizaÃ§Ã£o com calendÃ¡rio</div><div className="text-xs text-muted-foreground">Atualiza compromissos automaticamente</div></div><Switch checked={integrationSettings.calendarSync} onCheckedChange={(checked) => setIntegrationSettings((prev) => ({ ...prev, calendarSync: checked }))} /></div><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">IntegraÃ§Ã£o com drive</div><div className="text-xs text-muted-foreground">Sincroniza arquivos enviados</div></div><Switch checked={integrationSettings.driveSync} onCheckedChange={(checked) => setIntegrationSettings((prev) => ({ ...prev, driveSync: checked }))} /></div><div className="space-y-2"><Label className="text-sm">Webhook</Label><Input value={integrationSettings.webhookUrl} onChange={(event) => setIntegrationSettings((prev) => ({ ...prev, webhookUrl: event.target.value }))} /></div><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">Acesso por API</div><div className="text-xs text-muted-foreground">Permite integraÃ§Ã£o via token</div></div><Switch checked={integrationSettings.apiAccess} onCheckedChange={(checked) => setIntegrationSettings((prev) => ({ ...prev, apiAccess: checked }))} /></div><div className="space-y-2"><Label className="text-sm">Token da API</Label><div className="flex gap-2"><Input value={integrationSettings.apiToken} onChange={(event) => setIntegrationSettings((prev) => ({ ...prev, apiToken: event.target.value }))} /><Button type="button" variant="outline" onClick={() => setIntegrationSettings((prev) => ({ ...prev, apiToken: generateApiToken() }))}><KeyRound className="h-4 w-4 mr-1" />Gerar</Button></div></div></div><Button className="mt-4" onClick={saveIntegrations} disabled={savingSection === "integrations"}>{savingSection === "integrations" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar integraÃ§Ãµes</Button></div>}
            {activeSection === "security" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">SeguranÃ§a</h2><div className="space-y-4"><div className="grid sm:grid-cols-2 gap-4"><div className="space-y-2"><Label className="text-sm">Senha atual</Label><Input type="password" value={securityForm.currentPassword} onChange={(event) => setSecurityForm((prev) => ({ ...prev, currentPassword: event.target.value }))} /></div><div className="space-y-2"><Label className="text-sm">Nova senha</Label><Input type="password" value={securityForm.newPassword} onChange={(event) => setSecurityForm((prev) => ({ ...prev, newPassword: event.target.value }))} /></div><div className="space-y-2 sm:col-span-2"><Label className="text-sm">Confirmar nova senha</Label><Input type="password" value={securityForm.confirmPassword} onChange={(event) => setSecurityForm((prev) => ({ ...prev, confirmPassword: event.target.value }))} /></div></div><Button variant="outline" onClick={changePassword} disabled={savingSection === "security"}>{savingSection === "security" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Alterar senha</Button></div></div>}
          </div>
        </div>
      </div>
    </AppLayout>
