...
const settingSections: { id: SettingSectionId; title: string; description: string; icon: typeof User }[] = [
  ...
  { id: "company", title: "Empresa", description: "Dados da organização", icon: Building2 },
  { id: "security", title: "Segurança", description: "Senha e autenticacao", icon: Shield },
  { id: "notifications", title: "Notificações", description: "Alertas do sistema", icon: Bell },
  { id: "appearance", title: "Aparência", description: "Tema e idioma", icon: Palette },
  { id: "integrations", title: "Integrações", description: "Conexões externas", icon: Globe },
];

const notificationLabels = [
  { key: "assignedTasks", title: "Novas tarefas atribuídas", desc: "Quando uma tarefa for atribuida a você" },
  { key: "dueSoon", title: "Prazo próximo", desc: "Alerta de vencimento" },
  { key: "newForms", title: "Novos formulários", desc: "Quando cliente enviar formulario" },
...
export default function ConfiguracoesPage() {
  ...
  const saveAppearance = async () => {
    ...
    setSavingSection(null);
    if (error) return toast.error("Erro ao salvar aparência.");
  ...
  const saveIntegrations = async () => {
    ...
    setSavingSection(null);
    if (error) return toast.error("Erro ao salvar integrações.");
  ...
  const changePassword = async () => {
    ...
    if (securityForm.newPassword !== securityForm.confirmPassword) return toast.error("Confirmação da senha inválida.");
  ...
  return (
    <AppLayout>
      <div className="space-y-6 max-w-5xl">
        <div><h1 className="font-heading text-2xl font-bold">Configurações</h1><p className="text-sm text-muted-foreground">Gerencie seu perfil e preferências do sistema</p></div>
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeSection === "notifications" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">Notificações</h2><div className="space-y-4">{notificationLabels.map((item) => <div key={item.key} className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">{item.title}</div><div className="text-xs text-muted-foreground">{item.desc}</div></div><Switch checked={notificationSettings[item.key]} onCheckedChange={(checked) => setNotificationSettings((prev) => ({ ...prev, [item.key]: checked }))} /></div>)}</div><Button className="mt-4" onClick={saveNotifications} disabled={savingSection === "notifications"}>{savingSection === "notifications" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar notificacoes</Button></div>}
            {activeSection === "appearance" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">Aparência</h2><div className="grid sm:grid-cols-2 gap-4"><div className="space-y-2"><Label className="text-sm">Tema</Label><Select value={appearanceSettings.themePreference} onValueChange={handleThemePreferenceChange}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="system">Sistema</SelectItem><SelectItem value="light">Claro</SelectItem><SelectItem value="dark">Escuro</SelectItem></SelectContent></Select>{savingThemePreference && <p className="text-xs text-muted-foreground">Salvando tema...</p>}</div><div className="space-y-2"><Label className="text-sm">Idioma</Label><Select value={appearanceSettings.languageCode} onValueChange={(value) => setAppearanceSettings((prev) => ({ ...prev, languageCode: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="pt-BR">Português (Brasil)</SelectItem><SelectItem value="en-US">English (US)</SelectItem><SelectItem value="es-ES">Espanhol</SelectItem></SelectContent></Select></div></div><div className="mt-4 flex items-center justify-between gap-3 border rounded-lg p-3"><div><div className="text-sm font-medium">Modo compacto</div><div className="text-xs text-muted-foreground">Reduz espaços da interface</div></div><Switch checked={appearanceSettings.compactMode} onCheckedChange={(checked) => setAppearanceSettings((prev) => ({ ...prev, compactMode: checked }))} /></div><Button className="mt-4" onClick={saveAppearance} disabled={savingSection === "appearance" || savingThemePreference}>{savingSection === "appearance" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar aparência</Button></div>}
            {activeSection === "integrations" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">Integrações</h2><div className="space-y-4"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">Sincronização com calendário</div><div className="text-xs text-muted-foreground">Atualiza compromissos automaticamente</div></div><Switch checked={integrationSettings.calendarSync} onCheckedChange={(checked) => setIntegrationSettings((prev) => ({ ...prev, calendarSync: checked }))} /></div><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">Integração com drive</div><div className="text-xs text-muted-foreground">Sincroniza arquivos enviados</div></div><Switch checked={integrationSettings.driveSync} onCheckedChange={(checked) => setIntegrationSettings((prev) => ({ ...prev, driveSync: checked }))} /></div><div className="space-y-2"><Label className="text-sm">Webhook</Label><Input value={integrationSettings.webhookUrl} onChange={(event) => setIntegrationSettings((prev) => ({ ...prev, webhookUrl: event.target.value }))} /></div><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">Acesso por API</div><div className="text-xs text-muted-foreground">Permite integração via token</div></div><Switch checked={integrationSettings.apiAccess} onCheckedChange={(checked) => setIntegrationSettings((prev) => ({ ...prev, apiAccess: checked }))} /></div><div className="space-y-2"><Label className="text-sm">Token da API</Label><div className="flex gap-2"><Input value={integrationSettings.apiToken} onChange={(event) => setIntegrationSettings((prev) => ({ ...prev, apiToken: event.target.value }))} /><Button type="button" variant="outline" onClick={() => setIntegrationSettings((prev) => ({ ...prev, apiToken: generateApiToken() }))}><KeyRound className="h-4 w-4 mr-1" />Gerar</Button></div></div></div><Button className="mt-4" onClick={saveIntegrations} disabled={savingSection === "integrations"}>{savingSection === "integrations" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar integrações</Button></div>}
            {activeSection === "security" && <div className="rounded-xl border bg-card p-6"><h2 className="font-heading font-semibold mb-4">Segurança</h2><div className="space-y-4"><div className="grid sm:grid-cols-2 gap-4"><div className="space-y-2"><Label className="text-sm">Senha atual</Label><Input type="password" value={securityForm.currentPassword} onChange={(event) => setSecurityForm((prev) => ({ ...prev, currentPassword: event.target.value }))} /></div><div className="space-y-2"><Label className="text-sm">Nova senha</Label><Input type="password" value={securityForm.newPassword} onChange={(event) => setSecurityForm((prev) => ({ ...prev, newPassword: event.target.value }))} /></div><div className="space-y-2 sm:col-span-2"><Label className="text-sm">Confirmar nova senha</Label><Input type="password" value={securityForm.confirmPassword} onChange={(event) => setSecurityForm((prev) => ({ ...prev, confirmPassword: event.target.value }))} /></div></div><Button variant="outline" onClick={changePassword} disabled={savingSection === "security"}>{savingSection === "security" && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Alterar senha</Button></div></div>}
          </div>
        </div>
      </div>
    </AppLayout>
