...
const stages = [
  ...
  { label: "Reunião Agendada", count: 3, color: "bg-grow-gold" },
  { label: "Proposta Enviada", count: 6, color: "bg-grow-emerald-light" },
  { label: "Negociação", count: 2, color: "bg-primary" },
            ...
            {stages.map((s, i) => (
              <div key={s.label} className="flex-1 flex flex-col items-center gap-1">
                ...
                <div
                  ...
                  style={{ height: `${Math.max(s.count * 8, 8)}px` }}
                />
                <span className="text-[10px] text-muted-foreground text-center leading-tight mt-1 hidden lg:block">
                  {s.label}
                </span>
              </div>
            ))}
